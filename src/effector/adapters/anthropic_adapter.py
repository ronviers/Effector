"""
LLM Adapter — wraps the Anthropic API in a strict system prompt
that enforces DASP-compliant JSON output.

Provides:
  - AnthropicAgent: callable that takes AgentRequest, returns AgentResponse
...
class AnthropicAgent:
    """
    Wraps the Anthropic claude-sonnet API to produce DASP-compliant AgentResponse objects.
    """
    ...
    def __call__(self, request: AgentRequest) -> AgentResponse:
        """Call the Anthropic API and parse the DASP-compliant response."""
        try:
            import anthropic
        ...
        message = client.messages.create(
            ...
            messages=[{"role": "user", "content": task_with_others}],
        )
...
def make_agent_callable(
    ...
    """Convenience factory for creating an Anthropic-backed DASP agent."""
    return AnthropicAgent(agent_id=agent_id, tool_registry=tool_registry, temperature=temperature)
